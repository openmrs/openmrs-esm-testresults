...
import { ObsRecord } from '../loadPatientTestData/loadPatientData';
...
export const useTimelineData = (patientUuid: string, panelUuid: string) => {
  ...
  console.log({ sortedObs, loaded, error, patientUuid, panelUuid });
  ...
  const timelineData = useMemo(() => {
    ...
    // console.log({ entries, times, parsed: parseTime(times) });

    return { data: { parsedTime: parseTime(times), rowData, panelName }, loaded: true };
